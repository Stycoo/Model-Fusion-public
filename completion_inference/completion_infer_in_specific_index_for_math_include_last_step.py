...
answers = []
completion_inputs = []
gt_last_step_completion_outputs = []
...
for data in input_data:
    ...
    if args.completion_id == 1:
        ...
        gt_last_step_completion_outputs.append(parts[-1])

    else:
        prompts.extend([prompt] * 2)
        answers.extend([answer] * 2)
        chosen_completion_input = '\n\n'.join(parts[:args.iteration_time-args.completion_id+1])
...
for i, output in enumerate(outputs):
    ...
    if args.completion_id == 1:
        gt_last_step_completion_output = gt_last_step_completion_outputs[i]
        output_data[prompt] = {'prompt': prompt, 'answer': answer, 'completion_inputs': [answer, completion_input], 
                                'completion_outputs':[gt_last_step_completion_output, generated_text], 'chosen_rejected': [1, chosen_rejected]}
    else:
