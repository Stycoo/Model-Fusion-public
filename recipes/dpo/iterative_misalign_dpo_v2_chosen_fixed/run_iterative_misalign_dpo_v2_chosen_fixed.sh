...
VERSION='iterative_misalign_dpo_v2_chosen_fixed'
...
do  
    ...
    OUTPUT_DIR="$PROJ_DIR/data/FuseChat-3.0-DPO-Data/$MODEL_ID/$VERSION/iter_$i"
    
    if [ $i -eq 1 ]; then
        MODEL=$PROJ_DIR/base_models/$MODEL_ID
    else
        MODEL="$PROJ_DIR/saves/model_fusion/$MODEL_ID/$DATA/$VERSION/iter_$((i-1))"
    fi
    ...
    while [ $JOB_INDEX -lt $NUM_SEEDS ]; do
        ...
        for (( CHUNK_ID=0; CHUNK_ID<CHUNK_NUM; CHUNK_ID++ )); do
            ...
            CUDA_VISIBLE_DEVICES=$GPU python $PROJ_DIR/completion_inference/completion_infer_in_specific_index_for_math_chosen_fixed.py \
    ...
    POST_PROCESS_TYPE=create_misaligned_dpo_chosen_fixed_dataset
    ...
    python $PROJ_DIR/completion_inference/build_dataset_v2.py \
        ...
        --iterative_id $i > "$CREATE_DPO_LOG_FILE" 2>&1
...
MODEL_NAMES=(iter_1 iter_2 iter_3) # iter_2 iter_3
DATATYPES=(aime math)
N=(16 1)
