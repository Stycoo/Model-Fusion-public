...
def preprocess_pairwise_completion_dataset(
    ...
    for i in range(len(examples["_prompt"])):
        ...
        chosen_ids = tokenizer(chosen_response, truncation=True, padding=False).input_ids + [tokenizer.eos_token_id]
        rejected_ids = tokenizer(rejected_response, truncation=True, padding=False).input_ids + [tokenizer.eos_token_id]
